...
static int client_recv_buffer(rdma_buffer_attr_vec & server_metadata_attrs)
{
	...
	ret = ibv_post_recv(client_qps[3] /* which QP */, //代码调用ibv_post_recv函数来提交接收工作请求。该函数接受一些参数，包括一个指向客户端QP（Queue Pair）的指针、一个指向接收工作请求的指针以及一个指向错误工作请求的指针。如果提交成功，函数将返回0，否则返回一个非零值
...
void wait_for_server_ready(rdma_buffer_attr_vec & server_metadata_attrs)
{
	...
	struct ibv_wc wc;
	int ret = -1;
    ret = process_work_completion_events(io_completion_channels[3], //process_work_completion_events函数来等待并处理两个工作完成事件。一个是发送工作请求的完成事件，另一个是接收服务器发送的缓冲区信息的完成事件。如果成功接收到两个工作完成事件，代码会打印服务器发送的缓冲区位置和凭证信息。
			&wc, 1);
...
{	
	...
	{	printf("i=%d\n",i);
		...
		/* Now we post it */
		// sleep(5);
		int ret = ibv_post_send(client_qps[2],  //函数将发送请求发送到RDMA队列中。
				&client_send_wr,
			&bad_client_send_wr);
		if (ret) {
		...
		if((i+1)%5==0)
		{
			ret = process_work_completion_events(io_completion_channels[2], 
			wc, 5);
			if(ret != 5) {
				rdma_error("We failed to get 2 work completions , ret = %d \n",
						ret);
