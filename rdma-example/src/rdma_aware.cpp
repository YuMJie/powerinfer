...
struct ibv_comp_channel * io_completion_channel[40];
struct context
{
    /* User parameters */
    int server;
...
int init_resources(struct context *ctx, struct rdma_addrinfo *rai)
{
    ...
    {   
        ...
        {
            ret = rdma_resolve_addr(ctx->conn_id[i], NULL, rai->ai_dst_addr, 1000);
            if (ret)
            {
    ...
    {   
        for(int i = 0; i < ctx->qp_count; i++)
        {
            ret = rdma_bind_addr(ctx->conn_id[i], rai->ai_src_addr);
            if (ret)
            {
...
int run_server(struct context *ctx, struct rdma_addrinfo *rai)
{
    ...
    for (i = 0; i < ctx->qp_count; i++)
    {   
        io_completion_channel[i] = ibv_create_comp_channel(ctx->conn_id[i]->verbs);//ibv_create_comp_channel函数创建一个完成通道（Completion Channel），用于接收I/O完成事件的通知。完成通道与RDMA设备相关联，因此我们使用cm_client_id->verbs来指定RDMA设备。
        
        ibv_cq *cq = ibv_create_cq(ctx->conn_id[i]->verbs /* which device*/,  //ibv_create_cq函数创建一个完成队列（Completion Queue），用于存储RDMA操作完成的通知。完成队列中的元数据被打包到一个叫做struct ibv_wc的结构体中，它包含了有关工作完成的详细信息。在RDMA世界中，一个I/O请求被称为“工作”。
			ctx->max_wr /* maximum capacity*/, 
