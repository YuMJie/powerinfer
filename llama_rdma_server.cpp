...
//实现本地从远程加载模型
static int send_server_metadata_to_client_LLM_vec_withvector_api(std::vector<ggml_tensor*> tensor_dsts )  //该函数用于向连接的客户端发送服务器端缓冲区的元数据。
{
	struct ibv_wc wc; //工作完成（work completion）结构体
	int ret = -1;
	/* Now, we first wait for the client to start the communication by 
...
static int send_server_metadata_to_client_LLM_vec_withvector_api_only_read(std::vector<ggml_tensor*> tensor_dsts )  //该函数用于向连接的客户端发送服务器端缓冲区的元数据。
{
	struct ibv_wc wc; //工作完成（work completion）结构体
	int ret = -1;
	/* Now, we first wait for the client to start the communication by 
...
struct llama_model_loader {
    ...
    void load_all_data_RDMA(struct ggml_context * ctx, llama_progress_callback progress_callback, void * progress_callback_user_data, llama_mlock * lmlock) {
        ...
        accept_client_connection_LLM_vec();
        //====rdma====
...
void init_rdma_idx(int num_layer,struct ggml_context *aux_ctx,int dim)
{   
    rdma_idx_vec = malloc(num_layer * sizeof  (struct ggml_tensor*)) ;
    for(int i=0;i<num_layer;i++)
    {   
        struct ggml_tensor * rdma_idx = ggml_new_tensor_1d(aux_ctx, GGML_TYPE_F32, dim);
        ggml_set_zero(rdma_idx);
        ((struct ggml_tensor * *) rdma_idx_vec)[i] = rdma_idx;
    }
    // struct ggml_tensor * tensor = ((struct ggml_tensor **) rdma_idx_vec)[num_layer-1];
}
            ...
            case LLM_ARCH_REFACT:
                {
                    ...
                    model.layers.resize(n_layer);
                    init_rdma_idx(n_layer,ctx,model.layers[0].gpu_idx->ne[0]);
